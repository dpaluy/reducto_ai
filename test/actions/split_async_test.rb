...
module Actions
  class SplitAsyncTest < Minitest::Test
    ...
    def test_posts_payload
      payload = { input: "doc", split_description: [{ name: "Intro" }], async: { priority: false } }
      stub_split_request(payload, { job_id: "job-123" })

      response = ReductoAI::Actions::SplitAsync.new(client: @client).call(**payload, split_rules: nil)
    ...
    def stub_split_request(payload, response_body)
